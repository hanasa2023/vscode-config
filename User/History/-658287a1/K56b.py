...
from nonebot.adapters.qq import Bot, GuildMessageEvent, MessageSegment

from .config import HELP, MAX_PICS, Config
from .utils import save_image, check_database, update_database, update_optional_status
...
get_a_image = on_regex(
    r'\/setu\s*(?P<tag>\S*)?\s*[x|*]*(?P<num>\d+)?[张|个|份]*', re.I, priority=2)
...
test = on_regex(r"/setu get", block=True)
...
async def _(bot: Bot, event: GuildMessageEvent):
    ...
    files = await save_image(config.id)
    for file in files:
        try:
            await bot.send(event, MessageSegment.file_image(data=file))
